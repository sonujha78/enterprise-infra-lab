...
## Notes
- `slapd` does not auto-start under Docker (no systemd); started manually via `service slapd start` after container boot.
- Root password was reset via `ldapmodify` with SASL EXTERNAL auth (`ldapi` socket) because the debconf-preseeded password during `slapd` install did not take effect as expected.
- Bootstrap LDIF: `ldap/bootstrap/01-structure.ldif`
- User passwords set individually via `ldappasswd` (not committed to git).

## Verification
`ldapsearch` against the base DN returned all 8 entries (2 OUs, 3 users, 2 groups) successfully:

```
# alice, People, example.local
...
# developers, Groups, example.local
